...
export const FormContentWrapper: FC = ({ children, className }) => {
    return (
        <div className={twMergeCustom('bg-backgroundMore px-4 vl:px-8 rounded-xl max-w-3xl w-full', className)}>
            {children}
        </div>
    );
};
...
export const FormBlockWrapper: FC = ({ children, className }) => {
    return (
        <div className={twMergeCustom('py-4 vl:py-8 border-b border-b-borderAccent last:border-b-0', className)}>
            {children}
        </div>
    );
};
